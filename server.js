...
function logTraffic() {
    exec(`cat /proc/net/dev|grep ${config.interface}:|awk '{printf $10}'`, function (err, out, code) {
        if (!err) {
            var time = new Date();
            fs.appendFileSync(`./traffic.out.log`, `${time.getTime()} ${out}\n`);
        }
    });
    exec(`cat /proc/net/dev|grep ${config.interface}:|awk '{printf $2}'`, function (err, out, code) {
        if (!err) {
            var time = new Date();
            fs.appendFileSync(`./traffic.in.log`, `${time.getTime()} ${out}\n`);
        }
    });
}
setInterval(logTraffic, 1000 * config.granularity);

function getTrafficArr(file) {
    var arr_traffic = fs.readFileSync(file, 'utf-8').match(/\d+ \d+\n/g);
    var ret = new Array();
    ...
    switch (req.url) {
        case '/query':
            ...
            res.end(JSON.stringify({
                out: getTrafficArr('./traffic.out.log'),
                in: getTrafficArr('./traffic.in.log')
            }));
            break;
        default:
